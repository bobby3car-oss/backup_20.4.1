...
import 'mood_repository_local.dart';

class MoodRepositorySync implements MoodRepository {
  static final MoodRepositorySync instance = MoodRepositorySync._internal();

  factory MoodRepositorySync() => instance;

  MoodRepositorySync._internal({
    MoodRepositoryLocal? local,
    SyncQueueLocal? queue,
    SyncService? syncService,
    FirebaseAuth? firebaseAuth,
    FirestoreClient? firestoreClient,
  })  : _local = local ?? MoodRepositoryLocal.instance,
        _queue = queue ?? SyncQueueLocal(fileName: 'mood_sync_queue.json'),
  ...
  final MoodRepositoryLocal _local;
  ...
  @override
  Stream<List<MoodEntry>> watchAll() => _local.watchAll();
  ...
  Future<void> upsert(MoodEntry entry) async {
    ...
    // ── Gamification: record mood log ──
    if (_gamification != null) {
      unawaited(_gamification!.recordActivity(mood: true));
    }
    ...
    try {
      ...
      await _queue.enqueue(
        SyncOp(
          id: 'mood_upsert_${normalized.id}_${now.microsecondsSinceEpoch}',
          collectionPath: 'patients/$uid/mood_entries',
          docId: normalized.id,
      ...
      if (kDebugMode) {
        debugPrint('[MoodRepositorySync] enqueue failed: $error');
  ...
  Future<void> delete(String id) async {
    ...
    try {
      ...
      await _queue.enqueue(
        SyncOp(
          id: 'mood_delete_${id}_${now.microsecondsSinceEpoch}',
          collectionPath: 'patients/$uid/mood_entries',
          docId: id,
      ...
      if (kDebugMode) {
        debugPrint('[MoodRepositorySync] delete enqueue failed: $error');
        debugPrint('$stackTrace');
      }
    }
  }
  ...
  @override
  Future<MoodEntry?> getById(String id) => _local.getById(id);
  ...
  Future<void> pullLatest() async {
    ...
    try {
      ...
      for (final remoteDoc in remoteDocs) {
        ...
        final item = MoodEntry.fromJson(<String, dynamic>{
          ...remoteMap,
      ...
      if (kDebugMode) {
        debugPrint('[MoodRepositorySync] pullLatest failed: $error');
        debugPrint('$stackTrace');
      }
    }
  }
