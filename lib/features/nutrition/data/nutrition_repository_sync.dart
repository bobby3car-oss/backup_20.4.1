...
import 'nutrition_repository_local.dart';

class NutritionRepositorySync implements NutritionRepository {
  static final NutritionRepositorySync instance =
      NutritionRepositorySync._internal();

  factory NutritionRepositorySync() => instance;

  NutritionRepositorySync._internal({
    NutritionRepositoryLocal? local,
    SyncQueueLocal? queue,
    SyncService? syncService,
    FirebaseAuth? firebaseAuth,
    FirestoreClient? firestoreClient,
  })  : _local = local ?? NutritionRepositoryLocal.instance,
        _queue =
            queue ?? SyncQueueLocal(fileName: 'nutrition_sync_queue.json'),
  ...
  final NutritionRepositoryLocal _local;
  ...
  @override
  Stream<List<NutritionEntry>> watchAll() => _local.watchAll();

  @override
  Future<void> upsert(NutritionEntry entry) async {
    ...
    // ── Gamification: record nutrition log ──
    if (_gamification != null) {
      unawaited(_gamification!.recordActivity(nutrition: true));
    }
    ...
    try {
      ...
      await _queue.enqueue(
        SyncOp(
          id: 'nutrition_upsert_${normalized.id}_${now.microsecondsSinceEpoch}',
          collectionPath: 'patients/$uid/nutrition',
          docId: normalized.id,
      ...
      if (kDebugMode) {
        debugPrint('[NutritionRepositorySync] enqueue failed: $error');
  ...
  Future<void> delete(String id) async {
    ...
    try {
      ...
      await _queue.enqueue(
        SyncOp(
          id: 'nutrition_delete_${id}_${now.microsecondsSinceEpoch}',
          collectionPath: 'patients/$uid/nutrition',
          docId: id,
      ...
      if (kDebugMode) {
        debugPrint('[NutritionRepositorySync] delete enqueue failed: $error');
        debugPrint('$stackTrace');
      }
    }
  }
  ...
  @override
  Future<NutritionEntry?> getById(String id) => _local.getById(id);
  ...
  Future<void> pullLatest() async {
    ...
    try {
      ...
      for (final remoteDoc in remoteDocs) {
        ...
        final item = NutritionEntry.fromJson(<String, dynamic>{
          ...remoteMap,
      ...
      if (kDebugMode) {
        debugPrint('[NutritionRepositorySync] pullLatest failed: $error');
        debugPrint('$stackTrace');
      }
    }
  }
