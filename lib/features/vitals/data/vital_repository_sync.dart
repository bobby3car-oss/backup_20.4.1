...
class VitalRepositorySync implements VitalRepository {
  ...
  VitalRepositorySync._internal({
    ...
    FirestoreClient? firestoreClient,
       ...
       _queue = queue ?? SyncQueueLocal(fileName: 'vitals_sync_queue.json'),
  ...
  Future<void> upsert(VitalEntry entry) async {
    ...
    try {
      ...
      await _queue.enqueue(
        SyncOp(
          id: 'vital_upsert_${normalized.id}_${now.microsecondsSinceEpoch}',
          collectionPath: 'patients/$uid/vitals',
          docId: normalized.id,
      ...
      if (kDebugMode) {
        debugPrint('[VitalRepositorySync] enqueue failed: $error');
  ...
  Future<void> delete(String id) async {
    ...
    try {
      ...
      await _queue.enqueue(
        SyncOp(
          id: 'vital_delete_${id}_${now.microsecondsSinceEpoch}',
          collectionPath: 'patients/$uid/vitals',
          docId: id,
      ...
      if (kDebugMode) {
        debugPrint('[VitalRepositorySync] delete enqueue failed: $error');
        debugPrint('$stackTrace');
      }
    }
  }
